...
// Basic Functions in Geometry
#define cd const double &
const double EPS = 1E-8, PI = acos (-1); //Epsilon, Pi
...
struct point {
	double x, y;
	explicit point (cd x = 0, cd y = 0) : x (x), y (y) {} // defaults to 0
	...
	point _rot90 () const { return point (y, -x); } //clockwise
	point rot (cd t) const { //by radians
